...
.store_mode:
    ...
    je .set_mode_04h
    cmp al, 0x06           ; mode 6 clears the same 16 KB aperture
    je .set_mode_04h
...
.clr13_loop:
    ...
    dec cx
    jnz .clr13_loop
    ; --- Mode 13h: program the IBM default 256-entry VGA DAC. See
    ; corduroy's handlers.asm for the rationale; same table, same
    ; 0x3C8/0x3C9 sequence. Real VGA resets the DAC on set-mode.
    push si
    xor al, al
    mov dx, 0x3C8
    out dx, al
    mov dx, 0x3C9
    push cs
    pop ds                 ; DS = CS = F000 so DS:SI reaches the table
...
; See bios/corduroy/handlers.asm for the same table + rationale.
; Generated by tools/gen-vga-dac.mjs.
; ============================================================
vga_dac_default:
    incbin "../corduroy/vga-dac.bin"
